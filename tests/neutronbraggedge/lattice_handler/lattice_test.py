...
class TestLattice(unittest.TestCase):
    ...
    def test_wrong_type_bragg_edge_array_raise_error(self):
        """Assert in Lattice - Type Error is raised when type of bragg edge array is not numpy"""
        _material = 'NewMaterial'
        _crystal_structure = 'FCC'
        _bragg_edge_array = [1, 2, 3]
        o_lattice = Lattice(material = _material,
                            crystal_structure = _crystal_structure,
                            bragg_edge_array = _bragg_edge_array)
        self.assertRaises(TypeError, o_lattice._match_bragg_edge_with_hkl)
